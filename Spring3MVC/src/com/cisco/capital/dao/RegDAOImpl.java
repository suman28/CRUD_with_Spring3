	...
	public void editUser(RegistrationVO registrationVO) {
		...
		//  ResultSet rs = null;
		  try {
			  ...
			  con = jdbcTemplate.getDataSource().getConnection();
			  String query = "UPDATE REGISTRATION_TABLE SET FIRST_NAME=?,LAST_NAME=?,EMAIL_ID=?,CONTACT_NO=?,USER_NAME=? WHERE USER_ID=?";
			  pst = con.prepareStatement(query); 
			  pst.setString(1, registrationVO.getFirstName());
			  pst.setString(2, registrationVO.getLastName());
			  pst.setString(3, registrationVO.getEmail());
			  pst.setString(4, registrationVO.getContactNo());
			  pst.setString(5, registrationVO.getUserName());
			  pst.setString(6, registrationVO.getUserId());
